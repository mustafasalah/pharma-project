...
import { setProduct } from "../../services/products";
import { notify } from "../../utility";
import ProductPhoto from "./ProductPhoto";

const ProductPopupForm = ({ showState }) => {
    let state = useHookstate(productFormState);
    const { data, errors } = state;
    DevTools(state).label("P\roduct Popup Form");
    ...
    return (
        <PopupForm
            ...
            onSubmit={async () => {
                const { status } = await setProduct(data.get());
                notify({
                    status,
                    successMsg: "Product Item has been added successfully!",
                    successCallback: () => {
                        // Clear the form data
                        state.set(productFormState);
        ...
        >
            <ProductPhoto
                ...
                label="Product Photo"
                name="product_photo"
                photoPath={data.photo.value}
                photoSize={data.photo_size.value}
            />
